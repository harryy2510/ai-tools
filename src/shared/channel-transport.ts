/**
 * Shared cross-channel transport contracts and pure presentation helpers.
 * Channel clients implement these method names so a thin messaging seam is wiring only.
 * Full channel packs remain source of truth for native-only operations.
 */
...
export function createLiveMessage(deps: LiveMessageDeps): LiveMessage {
	...
	return {
		start: async (text: string) => {
			if (initialSendOutcomeUnknown !== undefined) throw initialSendOutcomeUnknown
			if (finalizing || messageId !== null) return
		...
		finalize: async (text: string) => {
			...
			if (lastAttemptedText === text && lastAttemptError !== undefined && deps.isOutcomeUnknown(lastAttemptError)) {
